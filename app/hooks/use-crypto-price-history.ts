...
/**
 * Hook to fetch real crypto price history from the database
 * @param cryptoId - The ID of the cryptocurrency to fetch history for
...
export function useCryptoPriceHistory(
  cryptoId: Id<"cryptocurrencies"> | null | undefined,
  limit?: number
) {
  const rawHistory = useQuery(
    api.crypto.getPriceHistory,
    cryptoId ? { cryptoId, limit } : "skip"
  );
